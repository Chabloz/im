...
Par soucis de performance, _Backbone_ ne retrie pas la collection automatiquement lorsqu'un attribut d'un modèle change (car il ne peut pas savoir lequel provoque un changement d'ordre). Toutefois, _Backbone_ propage les événements de changement des attributs d'un modèle sur la collection elle-même. Ainsi, il est possible d'écouter très simplement le changement de l'attribut de la date de début d'un cours et forcer le tri de la collection lorsque le changement se produit en appelant directement la méthode [sort](https://backbonejs.org/#Collection-sort). 


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4Nzg1OTYzNTMsMTk3MTE4MjUwNCwyOT
